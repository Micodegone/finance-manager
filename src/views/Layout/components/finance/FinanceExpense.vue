<template>
  ...
  <el-card class="financial-card">
    <template #header>
      <div class="card-header">
        ...
        <div class="search-container">
          ...
          <el-button @click="showAddDialog">添加支出</el-button>
          <el-tag type="success">本月总支出 ¥{{ formatAmount(currentMonthTotal) }}</el-tag>
        </div>
      </div>
    </template>
          ...
          <template #header>
            <div>
              <el-button :type="activeButton === '15days' ? 'primary' : ''"
                @click="handleTimeRange('15days')">近15日支出</el-button>
              <el-button :type="activeButton === 'month' ? 'primary' : ''"
                @click="handleTimeRange('month')">本月支出</el-button>
            </div>
          </template>
    ...
    <!-- 添加收入对话框 -->
    <el-dialog v-model="dialogVisible" title="添加收入" width="500px">
      <el-form :model="newData" :rules="dataRules" ref="formRef" label-width="80px">
        ...
        <el-form-item label="类别" prop="category" required>
          <el-select v-model="newData.category" @blur="formRef.validateField('category')" placeholder="请选择类别" filterable
            allow-create>
        ...
        <el-form-item label="金额" prop="amount" required>
          <el-input-number v-model="newData.amount" @blur="formRef.validateField('amount')" :min="0" :precision="2"
            style="width: 100%" />
        </el-form-item>
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `${params[0]?.axisValue}<br/>支出：¥${value.toFixed(2)}`
      }
    },
    xAxis: {
      type: 'category',
...
const categoryOptions = computed(() => {
  return [...new Set(expenseInfo.value.map(item => item.category))]
})

// 过滤后的数据
const filteredData = computed(() => {
  return expenseInfo.value.filter(item => {
    const date = new Date(item.date)
...
const {
  ...
  submitData,
  handleUpload,
  showAddDialog } = dataButton(expenseInfo, expenseStore)
