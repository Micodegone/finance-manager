<template>
  <div class="income-container">
      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="16">
        <el-card class="list-card" shadow="hover">
          ...
          <el-table v-else :data="tableData" stripe style="width: 100%;height: 56vh;" empty-text="暂无数据">
            ...
            <el-table-column prop="voucher" label="凭证" :min-width="100">
              <template #default="{ row }">
    ...
    <el-dialog v-model="dialogVisible" title="编辑收入记录" width="500px">
      <recordForm :initial-data="currenExpense" :categories="Object.keys(categories)" @submit="submitUpdate" />
    </el-dialog>
  </div>
</template>
...
import { useExpenseOperations } from '@/utils/expenseOpration'

const { handleDelete, handleUpdate } = useExpenseOperations()
const expenseStore = useExpenseStore()
const { expenseInfo, categories } = storeToRefs(expenseStore)

const dialogVisible = ref(false)
const currenExpense = ref(null)

// 打开编辑对话框
const openEditDialog = (row) => {
  currenExpense.value = { ...row }
  dialogVisible.value = true
}
// 提交更新
const submitUpdate = async (formData) => {
  try {
    await handleUpdate(currenExpense.value.id, formData)
...
const useTotalPieChart = totalPieChart(expenseInfo, pieChart)
const { initPieChart, updatePieChart, handleResize, destroyChart } = useTotalPieChart

//列表数据显示
const useTotalListData = totalListData(expenseInfo)
const {
    total,
    tableData,
    currentPage,
...
const refreshData = async () => {
  try {
    loading.value = true
    await expenseStore.getExpenseInfo()
    updatePieChart()
  }
  finally {
    loading.value = false
